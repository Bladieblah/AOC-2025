...
pub fn build(b: *std.Build) void {
    ...
    // ---- GENERATOR ----
    const gen_mod = b.addModule("generate_mod", .{ .root_source_file = b.path("template/generate.zig"), .target = target, .optimize = optimize });
    ...
    // ---- test_all ----
    const all_mod = b.addModule("all_tests_mod", .{ .root_source_file = b.path("src/test_all.zig"), .optimize = optimize, .target = target });
